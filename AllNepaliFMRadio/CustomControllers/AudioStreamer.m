...
//  Copyright 20013 Matt Gallagher. All rights reserved.
//
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (length > 0)
		{
			...
			if (myData->metaDataInterval == 0)
			{
				...
				if (statusCode == 200 || statusCode == 301 || statusCode == 302 || statusCode == 303 || statusCode == 307)		// "OK" (this is true even for ICY)
				{
			...
			if (myData.foundIcyStart && !myData.foundIcyEnd)
			{
				...
				while (YES)
				{
					...
					if (c1 == '\r' && c2 == '\n')
					{
						...
						NSLog(@"Header Line: %@. Length: %lu", line, (unsigned long)[line length]);
			...
			if (myData.parsedHeaders)
			{
				...
				for (int i=streamStart; i < length; i++)
				{
					...
					if (myData->metaDataBytesRemaining > 0)
					{
						...
						[myData.metaDataString appendFormat:@"%hhu", bytes[i]];
